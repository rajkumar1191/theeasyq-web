...
export async function createPost(slug, frontMatter, content) {
  // Optionally upload content as file to Cloud Storage
  const file = bucket.file(`posts/${slug}.md`);
  await file.save(content, { contentType: "text/markdown" });

  const postData = {
    ...frontMatter,
    contentPath: `posts/${slug}.md`, // store reference
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
  };

  await db.collection(COLLECTION).doc(slug).set(postData);
 ...
 * Update a post
 */
export async function updatePost(slug, frontMatter, content) {
  if (content) {
    const file = bucket.file(`posts/${slug}.md`);
    await file.save(content, { contentType: "text/markdown" });
  }
  ...
  await db.collection(COLLECTION).doc(slug).update(updateData);
  return { slug, ...updateData };
}
...
export async function deletePost(slug) {
  ...
  await bucket.file(`posts/${slug}.md`).delete().catch(() => {});
  return { slug };
}
